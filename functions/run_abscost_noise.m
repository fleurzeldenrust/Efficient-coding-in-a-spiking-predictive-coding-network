function [estimate, spikesideal, V, Thvec] = lew_run_abscost_noise(dt, si, g, tg, gin, gout, Th, nu, delta, aamp, atau, noisematrix, options)
...
% atau and size aamp.
% * noisematrix (Nneuron x length si): matrix with noise to each neuron
% * options
...
%% Make signal to each neuron
inputmat = zeros(Nker,Ntime);
for nn = 1:Nker
    inputmat(nn,:) = noisematrix(nn,:)+si;
end

%% Run network
V=zeros(Nker,Ntime);                 % 'membrane potential;': sum of effects of all filters on 1 neuron
Thvec=repmat((Th + nu*ones(size(Th)))',1,Ntime); % Threshold matrix; includes spike cost and adaptation
...
for i=1:Nker
   siconv(i,:)=convolve_kernel_acausal(inputmat(i,:), gin(i,:), 0, lg-1)*dt;
end
...
for tn = round(delta/dt)+1:Ntime
    ...
    if max(V(:,tn)-Thvec(:,tn))>0
        ...
        for ns = 1:length(nvec)
            ...
            % update Thvec
            if tn + la < Ntime
                Thvec(nn,tn+1:tn+la) = Thvec(nn,tn+1:tn+la) + thker;
            else
                Thvec(nn,tn+1:end) = Thvec(nn, tn+1:end) + thker(1:ll);
            end
        end
